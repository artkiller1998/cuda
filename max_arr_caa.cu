...
__global__ void find_kernel(int *c, int *a)
{ 
...
{	
	...
	long long int temp_count = count;
	int temp_val = 0;
	limit = 1024*65000;
	int delta = 0;

	while (temp_count > limit) {
		int *temp_arr = new int [limit];
		memcpy(temp_arr, array+delta, sizeof(int)*limit);
		cudaError_t cudaStatus = find_max(res_array, temp_arr, limit);
		...
		temp_count -= limit;
		delta += limit;
		if (temp_val < res_array[0])
			temp_val = res_array[0];
		
		if (temp_count <= limit) 
		{
			int *temp_arr = new int [limit];
			memcpy(temp_arr, array+delta, sizeof(int)*(count - delta));
			cudaError_t cudaStatus = find_max(res_array, temp_arr, limit);
			if (cudaStatus != cudaSuccess) {
				fprintf(stderr, "find_max failed!");
				return 1;
...
cudaError_t find_max(int *c, int *a, long long int size)
{
	...
	cudaStatus = cudaMalloc((void**)&dev_c, BLOCK_CNT * sizeof(int));
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMalloc failed!");
        goto Error;
    }
	...
	find_kernel<<<BLOCK_CNT,BLOCK_SIZE>>>(dev_c, dev_a);
	...
	while (BLOCK_CNT != 1)
	{			
		find_kernel<<<BLOCK_CNT,BLOCK_SIZE>>> (dev_c, dev_c);
